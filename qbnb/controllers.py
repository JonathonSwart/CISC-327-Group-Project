    ...
    if 'logged_in' in session:
        if request.method == "POST":
            ...
            if valid_listing is True:
                return render_template('create_listing.html', message="SUCCESS: Listing Created")
            else:
                return render_template('create_listing.html', message='FAILED: Try again with different inputs')
